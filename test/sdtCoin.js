...
/**
 * Way contract learning test
 *   deploy in before, and no deploy between it()
 */
contract("WayCoin", (accounts) => {
  ...
  const NOOFTOKENS = 4700000000; // 4.7B tokens
  ...
  let NoOfTokensInWei, OneWayInWei, NoOfTokensInWAY;
  let coin
  ...
  before(async () => {
    ...
    OneWayInWei = BIG(await coin.getOneWay());
    NoOfTokensInWAY = NoOfTokensInWei.div(OneWayInWei)
  });
  ...
  it("coin initial values", async () => {
    ...
    assert.equal(await coin.symbol(), "WAY");
    assert.equal(await coin.name(), "Who Are You");
    assert.equal(await coin.decimals(), 18);
    assert.equal((await coin.totalSupply()).toString(), BIG(NOOFTOKENS).mul(OneWayInWei).toString());
  ...
  it("token distribution", async () => {
    ...
    await coin.transfer(admin, OneWayInWei, {from: vault});
    assert.equal(await balanceOf(admin), OneWayInWei.toString());

    // transfer 0.5 token from admin to owner
    let halfPlayX = OneWayInWei.div(BIG(2))
    ...
    assert.equal(await balanceOf(vault), (NoOfTokensInWAY.sub(BIG(1)).mul(OneWayInWei).toString()));
  });
  ...
  it("marginal transfers", async () => {
    ...
    try {
      await coin.transfer(user2, OneWayInWei.mul(BIG(1)), {from: user1});
      assert.fail();
    }
    ...
    try
    {
      await coin.transfer(user2, OneWayInWei.mul(BIG(0)), {from: user1});
      assert.equal(await balanceOf(user1), OneWayInWei.mul(BIG(0)).toString());
      assert.equal(await balanceOf(user2), OneWayInWei.mul(BIG(0)).toString());
    }
    catch(exception) { assert.fail(); }

    // transfer 0 token from vault(balance > 0) to user1
    try {
        await coin.transfer(user1, OneWayInWei.mul(BIG(0)), {from: vault});
        assert.equal(await balanceOf(user1), OneWayInWei.mul(BIG(0)).toString());
    } catch(exception) { assert.fail(); }

    // transfer whole balance to other
    await coin.transfer(user1, OneWayInWei.mul(BIG(1)), {from: vault});
    assert.equal(await balanceOf(user1), OneWayInWei.mul(BIG(1)).toString());
    await coin.transfer(user2, OneWayInWei.mul(BIG(1)), {from: user1});
    assert.equal(await balanceOf(user1), OneWayInWei.mul(BIG(0)).toString());
    assert.equal(await balanceOf(user2), OneWayInWei.mul(BIG(1)).toString());
  });
  ...
  it("reserve and reserveOf", async () => {
    ...
    await coin.transfer(user1, OneWayInWei.mul(BIG(10)), { from: vault });
    await coin.setReserve(user1, OneWayInWei.mul(BIG(5)), { from: admin });
    assert.equal(await reserveOf(user1), OneWayInWei.mul(BIG(5)).toString());
  });
  
  it("transfer after setting reservation", async () => {
    // user1 = 10 WAY
    await coin.transfer(user1, OneWayInWei.mul(BIG(10)), { from: vault });
    // user1 = 10 WAY with 5 WAY reserve
    await coin.setReserve(user1, OneWayInWei.mul(BIG(5)), { from: admin });
    // user1 = 9 WAY with 5 WAY reserve
    await coin.transfer(user2, OneWayInWei.mul(BIG(1)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneWayInWei.mul(BIG(9)).toString());

    // user1 = 6 WAY with 5 WAY reserve
    coin.transfer(user2, OneWayInWei.mul(BIG(3)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneWayInWei.mul(BIG(6)));

    try {
      let tx = await coin.transfer(user2, OneWayInWei.mul(BIG(2)), {from: user1}); // violating reserved amount
      assert.equal(await balanceOf(user1), OneWayInWei.mul(BIG(6)));
      assert.fail();
    }
    catch(exception) {
